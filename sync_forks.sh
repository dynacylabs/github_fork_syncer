...
BASE_REPO_DIR="${REPO_BASE_DIR:-/workspaces/github_fork_syncer/repos}"

# Create the base directory if it doesn't exist
mkdir -p "$BASE_REPO_DIR"

# Get a list of your forks and their upstream sources, along with their default branch
echo "Fetching all repositories for user: $GITHUB_USERNAME"
...
if [ -z "$FORK_NAMES" ]; then
    echo "No fork repositories found for user: $GITHUB_USERNAME"
    exit 0
fi
...
while IFS= read -r REPO_NAME; do
    ...
    if [ -n "$PARENT_FULL_NAME" ] && [ "$PARENT_FULL_NAME" != "null" ]; then
        ...
        REPOS="${REPOS}${REPO_NAME} ${PARENT_FULL_NAME} ${PARENT_DEFAULT_BRANCH}\n"
    else
...
if [ -z "$REPOS" ]; then
    echo ""
    echo "No valid fork repositories with upstream found."
    echo "This could mean your forks are orphaned (upstream deleted) or there are API issues."
    exit 0
fi

echo ""
echo "Valid fork repositories with upstream:"
echo -e "$REPOS"
...
echo -e "$REPOS" | while IFS= read -r line; do
    ...
    # Parse the line (format: "REPO UPSTREAM DEFAULT_BRANCH")
    read -r REPO UPSTREAM DEFAULT_BRANCH <<< "$line"
    
    # Skip if we don't have all required information
    if [ -z "$REPO" ] || [ -z "$UPSTREAM" ] || [ -z "$DEFAULT_BRANCH" ]; then
        echo "Skipping invalid repository data: '$line'"
        continue
    fi
    
    echo "Processing fork: $REPO from upstream: $UPSTREAM, default branch: $DEFAULT_BRANCH"
    
    # Set the repository directory path
    REPO_DIR="$BASE_REPO_DIR/$REPO"
    
    # Check if repository directory exists, if not, clone it
    if [ ! -d "$REPO_DIR" ]; then
        echo "Repository directory not found. Cloning $REPO..."
        cd "$BASE_REPO_DIR"
    ...
    if [[ ! "$ORIGIN_URL" =~ github\.com[/:]${GITHUB_USERNAME}/${REPO}(\.git)?$ ]]; then
        echo "Warning: Origin URL doesn't match expected fork URL for $REPO"
        echo "Expected pattern: github.com/${GITHUB_USERNAME}/${REPO}"
    ...
    if [ $PUSH_EXIT_CODE -eq 0 ]; then
        ...
        if echo "$PUSH_OUTPUT" | grep -q "workflow.*scope"; then
            echo "⚠️ Synced $REPO but couldn't push due to workflow permissions"
